...
function MultitextResultsPlaceholder(props) {
  const { asyncReady, fetchResults, getSearchStatus, multitextSelections,
          multitextResults, results, searchID, searchProgress,
          searchStatus } = props;

  /** CSS styles and global theme. */
  const classes = useStyles(props);

  const searchInProgress = searchID && results.length === 0 && mul;
